...
public class AdminController {
    ...
    public ResponseEntity<Object> createIdentity(@RequestBody Map<String, String> body) {
        String email = body.get("email");
        String name = body.get("name");
        String password = body.get("password");

        if (email == null || name == null || password == null) {
            return new ResponseEntity<>("email, name, and password are required", HttpStatus.BAD_REQUEST);
        }

        Identity identity = adminService.registerUser(email, name, password);
